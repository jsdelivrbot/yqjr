...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 页面初始化 设置div：mainPage 的宽高 保证当页面高度超过可视窗口 div：mainPage出现滚动条
            // div：mainPage的宽度：最外层宽度 - 左边导航的宽度（270）
            // div：mainPage的高度：最外层高度 - 上边导航的高度（100）
            setPageParent () {
            ...
            init () {
                this.setPageParent();
                this.userForm.name = JSON.parse(Cookies.get('user')).name;
                this.userForm.cellphone = JSON.parse(Cookies.get('user')).mobile;
                this.initPhone = '17712345678';
                this.userForm.company = JSON.parse(Cookies.get('user')).comName;
                this.userForm.department = JSON.parse(Cookies.get('user')).areaName;
            },
            cancelInputCodeBox () {
