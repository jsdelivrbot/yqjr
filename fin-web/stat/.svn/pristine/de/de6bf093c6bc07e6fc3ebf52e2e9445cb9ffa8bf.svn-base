            ...
            <Row>
                <Col span="12" class="demo-tabs-style1" style=" background: #e3e8ee;padding:16px; width: 100%">
                    <Tabs type="card" :animated="false">
                        <TabPane label="预警详情">
                            <Table stripe :data="teamMainQueryPersonList" :columns="personTableColumns"
                                   :loading="loading" ref="tableExcel" class="margin-top-10"></Table>
                            <Table stripe :data="teamMainQueryPersonList" :columns="xinxiList" :loading="loading"
                                   ref="tableExcel" class="margin-top-10"></Table>
                            <Table stripe :data="teamMainQueryPersonList" :columns="calformulaList" :loading="loading"
                                   ref="tableExcel" class="margin-top-10"></Table>
                            <p style="text-align: center;font-size: 14px; font-family:'微软雅黑';font-weight: bold">
                                干预记录
                            </p>
                            <Table stripe :data="IndicatorLogsTabList"
                                   @on-row-click="chaKanXiangQing(IndicatorLogsTabList,$event)"
                                   :columns="IndicatorLogsList" :loading="loading" ref="tableExcel"
                                   class="margin-top-10"></Table>
                            <Row class="margin-top-20">
                                <Col span="4" offset="9">
                                    <Button type="primary" icon="ios-search" @click.prevent="ell()"
                                            style="margin-right: 70px">反馈审核结论
                                    </Button>
                                    <Modal ref="rr" v-model="modal3" :mask-closable="false">
                                        ...
                                        <Form ref='indicatorLogss' :model='indicatorLogss' :label-width='100'>
                                            <FormItem prop="reason">
                                                <textarea v-model="indicatorLogss.asstest" autoFocus
                                                          style="overflow:auto;width: 125%; resize: none;margin-left:-100px ;height:200px"></textarea>
                                            </FormItem>
                                        </Form>
                                        <div slot="footer" style="font-size: 14px ;font-weight: bold">
                                            <Button type="ghost" size="large" @click="cancleSubmit()"
                                                    style="margin-right: 40px">取消
                                            </Button>
                                            <Button type="primary" size="large" @click.prevent="queren()"
                                                    style="margin-right: 40px">确认
                                            </Button>
                                        </div>
                                    </Modal>
                                </Col>
                                <Col span="6">
                                    ...
                                    <Modal ref="rr" v-model="modal4" :mask-closable="false">
                                        ...
                                        <textarea v-model="indicatorLogss.xiangqing" autoFocus
                                                  style="overflow:auto;width: 100%; resize: none;margin-left:0px ;height:200px;"
                                                  readonly></textarea>
                                        <div slot="footer" style="font-size: 14px ;font-weight: bold">
                                            <Button type="ghost" size="large" @click="cancleSubmit1()"
                                                    style="margin-right: 40px">返回
                                            </Button>
                                        </div>
                                    </Modal>
                                </Col>
...
<script>
    ...
    export default {
            ...
            return {
                ...
                IndicatorLogsList: [{
                    ...
                    render: (h, params) => {
                        const row = params.row;
                        return h('div', [
                            h('div', row.operationtype === '1' ? '审查反馈' : row.operationtype === '2' ? '干预反馈' : row.operationtype === '3' ? '沟通反馈' : row.operationtype === '4' ? '回复反馈' : row.operationtype === '5' ? '提交审核反馈' : row.operationtype === '6' ? '审核结果反馈' : row.operationtype === '7' ? '撤回反馈' : row.operationtype === '0' ? '取消反馈' : '')
                        ]);
                    }
                }, {
                    title: '操作者',
        ...
        methods: {
            ...
            queren () {
                console.log(this.indicatorLogss.asstest);
                console.log(this.indicatorLogss.asstest.indexOf(''));
                if (!isEmpty(this.indicatorLogss.asstest.trim())) {
                    const user = getUserCookie();
                    const roleCode = user.roleCodeList;
                    // 预警业务经理
                    if (roleCode.indexOf('0008') >= 0) {
                        this.indicatorLogss.handlestate = 5;
                        this.indicatorLogss.operationtype = 6;
                    }
                    // 区域经理
                    if (roleCode.indexOf('0003') >= 0) {
                        this.indicatorLogss.handlestate = 3;
                        this.indicatorLogss.operationtype = 4;
                    }
                    ...
                    this.updatedate.eventid = this.$route.query.eventid;
                    this.updatedate.handlestate = this.indicatorLogss.handlestate;
