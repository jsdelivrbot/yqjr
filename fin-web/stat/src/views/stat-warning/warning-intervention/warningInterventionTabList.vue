                            ...
                            <Tabs type="card" :animated="false">
                                <TabPane label="预警详情">
                                    ...
                                    <Row class="margin-top-20">
                                        <Col span="4" offset="9">
                                            <Button type="primary" icon="ios-search" @click.prevent="ganYuFanKui()" style="margin-right: 70px">干预反馈</Button>
                                        ...
                                        <Col span="6" >
                                            ...
                                            <Modal ref="rr" v-model="modal4" :mask-closable="false">
                                                ...
                                                <Form ref='indicatorLogss'  :model='indicatorLogss'  :label-width='100'>
                                                <textarea v-model="indicatorLogss.xiangqing"  autoFocus style="overflow:auto;width: 100%; resize: none;margin-left:0px ;height:200px;"></textarea>
                                                </Form>
                                                <div slot="footer" style="font-size: 14px ;font-weight: bold">
                                                    <Row  >
                                                        <Col span="8" offset="2" v-show="showFlag.areaFlag">
                                                            <Button type="primary" size="large"  @click.prevent="xiugai()" style="margin-right: 40px">确认修改</Button>
                                                        </Col>
                                                         <Col span="8" offset="2">
...
<script>
    ...
    export default{
            ...
            return {
                ...
                IndicatorLogsList: [{
                    ...
                    render: (h, params) => {
                        const row = params.row;
                        return h('div', [
                            h('div', row.operationtype === '1' ? '审查反馈' : row.operationtype === '2' ? '干预反馈' : row.operationtype === '3' ? '沟通反馈' : row.operationtype === '4' ? '回复反馈' : row.operationtype === '5' ? '提交经理反馈' : row.operationtype === '6' ? '审核结果反馈' : row.operationtype === '0' ? '取消反馈' : row.operationtype === '7' ? '撤回反馈' : '')
                        ]);
                    }
                }, {
                    title: '操作者',
        ...
        methods: {
            ...
            queren () {
                ...
                if (roleCode.indexOf('0009') >= 0) {
                    if (!isEmpty(this.indicatorLogss.asstest.trim())) {
                        ...
                        // this.searchFormDate.eventid= this.$route.query.eventid;
                        // this.searchFormDate.handlestate=3;
                        this.updatedate.eventid = this.$route.query.eventid;
                        this.updatedate.handlestate = 3;
            ...
            xiugai () {
                if (!isEmpty(this.indicatorLogss.xiangqing.trim())) {
                    this.updateIndicatorLogss(this.indicatorLogss).then(res => {
                        resetWorkHeight();
                        this.modal4 = false;
            ...
            chaKanXiangQing (data, event) {
                ...
                this.indicatorLogss.id = event.id;
                if (event.operationtype == 5) {
