...
<template>
    <div class="main" :class="{'main-hide-text': shrink}" id="grandParent">
        <div class="sidebar-menu-con" id="leftNav" :style="{width: shrink?'60px':'270px', overflow: shrink ? 'visible' : 'auto',background: menuTheme=='dark'?'#495060':'#fff'}">
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                leftNavWidth: '', // 左边导航的宽度 也是右边内容区域的居左距离
                shrink: false,
                loginName: '',
                isFullScreen: false,
        ...
        methods: {
            ...
            // iframe的父亲页面 与 首页home的父亲页面 是并列的关系 一次只能存在一个 所以条件判断
            setChildPage () {
            ...
            handleSubmenuChange (name) {
                ...
                if (this.menuArr[0] === name) {
                    if (document.getElementById('show-iframe') !== null) {
                        // 重新给iframe赋值src
                        document.getElementById('show-iframe').src = document.getElementById('show-iframe').src;
                        // 重新刷新iframe页面 使其点击自己时页面实现刷新
                        // window.frames[0].postMessage({
                        //     act: 'reload',
                        //     msg: {
                        //         url: document.getElementById('show-iframe').src
                        //     }
                        // }, '*');
                    }
                }
