                ...
                <Row>
                    <Col span="8" offset="2">
                        <FormItem label="人员名称：" prop="personName">
                            <Select  :label-in-value="true" filterable remote :remote-method="searchPerson"  @on-change='getPersonCode' :loading="loading" placeholder="请输入人员姓名|自动搜索" v-model="formValidate.personName">
...
export default{
    name: 'personEventAdd',
    data () {
        const validateEventContentCheck = (rule, value, callback) => {
            if (value.length > 265) {
        ...
        return {
            ruleValidate: {
                ...
                brandsText: [
                    { validator: validateEventContentCheck }
                ]
            },
            file: null,
            loadingStatus: false,
