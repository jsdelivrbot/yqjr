...
public class CommodityReviewService extends BaseService<CommodityReviewMapper, CommodityReview, Long> {
    ...
    public void outReview(CommodityReview review){
        ...
        if("04".equals(commodityStatus)){//审核通过
            ...
            for(int i=0;i<commoReviews.size();i++){
                ...
                commodityOut.setReceiveStatus("1");
     ...
     * 补库存
     */
    public void inStock(String batchCode) {
        //通过批次号获取经销商数据
        List<CommodityReview> commodityReviews =selectByBatchCode(batchCode);
        Long sum = (long)0;
        //计算出了多少数量
     ...
     * @return
     */
    public List<CommodityReview> queryOutReviewList(CommodityReviewCondition commodityReviewCondition, PageBounds... pageBounds) {
        List<CommodityReview> commodityReviews = commodityReviewMapper.queryOutReviewList(commodityReviewCondition, pageBounds);
