            ...
            <Card>
                ...
                <Form ref="searchFormDate"  :model="searchFormDate" :label-width="100">
                    <Row>
                        ...
                        <Col span="8" offset="2">
                            <FormItem label="人员名称:">
                                <Input v-model="searchFormDate.personName" placeholder="请输入人员名称"></Input>
                            </FormItem>
                        </Col>
                    </Row>
                    <Row>
        ...
        <Row class="margin-top-10">
            <Card>
                ...
                <!--<a href="#" slot="extra" @click.prevent="exportExcel">-->
                    ...
                    <!--导出数据-->
                <!--</a>-->
                <Table :data="personReviewProgressData" :columns="tableColumns"  :loading="loading" ref="tableExcel" class="margin-top-10"></Table>
...
export default{
                        ...
                        if (reviewStatus === '00') {
                            return h('div', [
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            let routerParam = {id: params.row.personTeamReviewId, pageType: 'personReviewProgressList', workId: params.row.flowId, personCode: params.row.personCode};
                                            this.$router.push({
                                ...
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            let routerParam = {
                                                id: params.row.personTeamReviewId,
                                                personCode: params.row.personCode
                                            };
                                            this.$router.push({
    ...
    computed: {
        ...mapState({
            loading: ({personReviewProgressList}) => personReviewProgressList.loading,
            pageNo: ({personReviewProgressList}) => personReviewProgressList.pageNo,
            personReviewProgressData: ({personReviewProgressList}) => personReviewProgressList.personReviewProgressData,
            pageSize: ({personReviewProgressList}) => personReviewProgressList.pageSize,
            total: ({personReviewProgressList}) => personReviewProgressList.total,
            pages: ({personReviewProgressList}) => personReviewProgressList.pages
        })
    },
    methods: {
        ...mapActions([
            'handleQueryPersonReviewProgressList'
        ]),
        /**
         * 初始化
         */
         ...
         */
        searchData () {
            this.handleQueryPersonReviewProgressList(this.searchFormDate).then(res => {
                resetWorkHeight();
