import {
    apiUpdatePersonStatus,
    apiPersonReviewToSubmit,
...
import {
    apiQueryBrandList, apiQueryCityList,
...
const personReviewEdit = {
    ...
    actions: {
        ...
        handlePersonReviewEditData ({ commit }, personCode) {
            commit('sendin');
            return new Promise((resolve, reject) => {
                apiQueryPersonReviewViewById(personCode).then((res) => {
                    commit('setPersonReviewEditData', res);
                    commit('success');
                    resolve();
                }).catch((err) => {
                    commit('success');
        ...
        handlePersonReviewBrandsList ({ commit }) {
            commit('sendin');
            return new Promise((resolve, reject) => {
                apiQueryBrandList().then((res) => {
                    commit('setQueryBrandListData', res);
                    commit('success');
                    resolve();
                }).catch((err) => {
                    commit('success');
        ...
        handelPersonReviewProvinceList ({ commit }) {
            commit('sendin');
            return new Promise((resolve, reject) => {
        ...
        handelPersonReviewCityList ({ commit }, data) {
            commit('sendin');
            return new Promise((resolve, reject) => {
                apiQueryCityList(data).then((res) => {
    ...
    mutations: {
        ...
        setPersonReviewEditData: function (state, data) {
            var brandArr = [];
            var cityArr = [];
            var provinceArr = [];
            if (data.isSuccess) {
                ...
                state.workFlowMes = {
                    type: data.result.workFlow.type
                };
                state.personReviewEditData = {
                    opinion: '同意',
        ...
        setQueryBrandListData (state, data) {
            if (data.isSuccess) {
