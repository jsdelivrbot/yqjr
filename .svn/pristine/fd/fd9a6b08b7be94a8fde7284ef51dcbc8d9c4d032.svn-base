...
public class IndicatorTypesRest extends BaseRest<IndicatorTypesService, IndicatorTypes> {
    ...
    public ResultMsg<String> DeleteIndicatorTypesRest(IndicatorTypes it) {
        try {
            logger.info("进入根据id删除指标类型");
            baseService.deleteById(it.getId());
            logger.info("结束根据id删除指标类型");
    ...
    public ResultMsg<String> SelectTypecodByCode(String idteam) {
        ...
        try {
            logger.info("进入根据code查询指标表有没有子类型并删除");
            int sum = 0;
            ...
            logger.info("结束根据code查询指标表有没有子类型并删除");
        } catch (Exception e) {
            r.setCode(Const.CODE_FAILURE);
            r.setMsg("出现错误");
            r.setIsSuccess(Const.FAILURE);
            logger.error("根据code查询指标表有没有子类型并删除",e);
        }
    ...
    @ApiOperation(value = "查询代码是否重复", tags = "用户信息：UserController")
    @RequestMapping(value = "/SelectByName", method = RequestMethod.GET)
    public ResultMsg<String> SelectByName(String code) {
            logger.info("进入查询代码是否重复");
            boolean a = baseService.SelectByName(code);
            logger.info("结束查询代码是否重复");
            if(a==true) {
                return new ResultMsg<String>(Const.CODE_SUCCESS, "指标代码可用", Const.SUCCESS, "");
            }else {
                return new ResultMsg<String>(Const.CODE_FAILURE, "指标代码重复", Const.FAILURE, "");
            }
