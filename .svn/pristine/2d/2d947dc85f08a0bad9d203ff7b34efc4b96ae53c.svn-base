package com.yqjr.fin.fpms.product.entity;

import com.ritoinfo.framework.evo.sp.base.starter.entity.BaseMapperEntity;
import com.yqjr.fin.common.base.starter.entity.FinBaseMapperEntity;
import lombok.Data;
import lombok.EqualsAndHashCode;
import org.apache.ibatis.type.JdbcType;
import tk.mybatis.mapper.annotation.ColumnType;
import tk.mybatis.mapper.annotation.KeySql;
import tk.mybatis.mapper.code.ORDER;

import javax.persistence.*;
import java.sql.Timestamp;
import java.util.Objects;

/**
 * @description: 金融专案品牌关系
 * @author: lixue
 * @date: 2018/08/22 09:14:11
 **/
@EqualsAndHashCode(callSuper = true)
@Data
@Table(name = "FPMS_PROJECT_BRANDS", schema = "FPMS")
public class ProjectBrandsEntity extends FinBaseMapperEntity<Long> {

    /**
     * 主键id
     */
    @Id
    @KeySql(sql = "select SEQ_FPMS_PROJECT_BRANDS.nextval from dual", order = ORDER.BEFORE)
    private Long id;

    /**
     * 金融专案id
     */
    @Column(name = "FINANCE_PROJECT_ID", nullable = true, precision = 0)
    @ColumnType(jdbcType = JdbcType.BIGINT)
    private Long financeProjectId;

    /**
     * 品牌编码
     */
    @ColumnType(jdbcType = JdbcType.VARCHAR)
    private String brandsCode;

    /**
     * 子品牌编码
     */
    @ColumnType(jdbcType = JdbcType.VARCHAR)
    private String subbrandCode;


    /**
     * 状态 1 有效 0 无效
     */
    @ColumnType(jdbcType = JdbcType.BIGINT)
    private Long flag;

}
