...
import com.yqjr.scfw.common.excel.adapter.IOAdapter;
import com.yqjr.scfw.common.excel.annotation.ExcelColumnGroup;
import com.yqjr.scfw.common.excel.annotation.ExcelVoConfig;
import com.yqjr.scfw.common.excel.bean.*;
import com.yqjr.scfw.common.excel.exception.AdapterException;
import com.yqjr.scfw.common.excel.exception.ColumnErrorException;
import com.yqjr.scfw.common.excel.exception.ExcelHeaderErrorException;
import com.yqjr.scfw.common.excel.exception.ExcelVoErrorException;
import com.yqjr.scfw.common.excel.json.JsonUtil;
import com.yqjr.scfw.common.excel.utils.*;
...
public class ImportExcelService<T extends BaseExcelVo> extends BaseExcelService {
    ...
    private void readHead() throws ExcelHeaderErrorException {
        ...
        try{//读取hash密码
            hashVal=getHashVal(sheet);
             ...
             throw new ExcelHeaderErrorException(Message.FLAG_ERROR,e);
        }
        List<String> columns = new ArrayList<String>(Const.LIST_DEFAULT_CAPACITY);
     ...
     * @param indexClo
     * @return ExcelVo对象
     * @throws com.yqjr.scfw.common.excel.exception.ColumnErrorException
...
//          log.debug("获取一行数据为："+JsonUtil.toJSON(excelVo));
        ...
        for (int i = 0; i < length; i++) {//循环体外
            try {
               ...
               throw (AdapterException) e;
                ...
                throw new ColumnErrorException(row+1, index+1, Message.COLUMN_ERROR,e);
            }
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                ...
                if (e instanceof AdapterException) {
                    ...
                    throw new ColumnErrorException(row+1,i+index+1, Message.COLUMN_ERROR,e);
                }
            }
        }
        dataBean.setFieldValue(fieldBean.getField().getName(), excelVo, list);
    ...
    private void setSimpleField(FieldBean fieldBean, BaseExcelVo excelVo, Row sheetRow, int row, int index, DataBean dataBean) throws AdapterException, ColumnErrorException {
        ...
        try {
            ...
            if (e instanceof AdapterException) {
                ...
                throw new ColumnErrorException(row+1, fieldBean.getField().getName(), Message.COLUMN_ERROR,e);
            }
        }
    }
     ...
     *
     * @return
     * @throws com.yqjr.scfw.common.excel.exception.ExcelVoErrorException
     */
...
//                log.error(e.getMessage(), e);
                ...
                ExcelVoErrorException errorException = new ExcelVoErrorException(voClass, Message.VO_INSTANCE_ERROR,e);
