...
//					log.debug("创建目标文件所在的目录失败!");
		...
		try {
			// 打开源文件
			Path pathIn = Paths.get(srcFileName);
			is = java.nio.file.Files.newInputStream(pathIn);
			// 打开目标文件的输出流
			Path pathOut = Paths.get(descFileName);
			os = java.nio.file.Files.newOutputStream(pathOut);
...
//			log.debug("复制文件失败：" + e.getMessage());
			...
			if (os != null) {
				try {
					...
					log.error(oute.getMessage(),oute);
				}
			}
			if (is != null) {
				try {
...
//			log.debug("文件压缩失败，目录 " + srcDirName + " 不存在!");
		...
		Path pathOut = Paths.get(descFileName);
		try {
			ZipOutputStream zouts = new ZipOutputStream(java.nio.file.Files.newOutputStream(pathOut));
	...
	public static boolean unZipFiles(String zipFileName, String descFileName) {
		...
		try {
			...
			while (enums.hasMoreElements()) {
				...
				// 打开文件输出流
				Path pathOut = Paths.get(descFileDir);
				OutputStream os = java.nio.file.Files.newOutputStream(pathOut);
...
//			log.debug("文件解压失败：" + e.getMessage());
	...
	public static void zipFilesToZipFile(String dirPath, File file, ZipOutputStream zouts) {
		...
		if (file.isFile()) {
			try {
				// 创建一个文件输入流
				fin = java.nio.file.Files.newInputStream(file.toPath());
				// 创建一个ZipEntry
