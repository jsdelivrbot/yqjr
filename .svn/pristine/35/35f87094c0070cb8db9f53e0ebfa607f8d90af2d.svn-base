...
$(function () {
  $("#activeDealerMatchTable").jqGrid(
    {
      ...
      colModel: [
        { label: 'ID', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '经销商活动上限数量', name: 'distributorCount', index: 'distributorCount', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeDealerMatchTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeDealerMatchTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeDealerMatchTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeDealerMatchTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeDealerMatch/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeDealerMatchTable").trigger("reloadGrid");
            });
        }
    }
}