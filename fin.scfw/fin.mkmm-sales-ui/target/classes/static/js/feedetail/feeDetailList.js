...
$(function () {
  ...
  $("#feeDetailTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#feeDetailTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#feeDetailTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#feeDetailTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/feeDetail/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#feeDetailTable").trigger("reloadGrid");
            });
        }
    }
}