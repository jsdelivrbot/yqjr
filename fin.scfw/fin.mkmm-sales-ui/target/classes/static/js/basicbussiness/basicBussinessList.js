...
$(function () {
  $("#basicBussinessTable").jqGrid(
    {
      ...
      colModel: [
        { label: 'ID', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '营销活动（车贷金融产品id）', name: 'nactivitiesId', index: 'nactivitiesId', sortable: true, width: 150, align: 'center' } ,
						        { label: '营销活动名称（车贷金融产品id）', name: 'nactivitiesName', index: 'nactivitiesName', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '金融产品(服务产品Code)', name: 'ndlrbizType', index: 'ndlrbizType', sortable: true, width: 150, align: 'center' } ,
						        { label: '金融产品(服务产品名称)', name: 'ndlrbizTypeName', index: 'ndlrbizTypeName', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '创建时间(数据提取时间)', name: 'createTime', index: 'createTime', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '活动统计标识（01-未统计 02-已统计）', name: 'activematchflag', index: 'activematchflag', sortable: true, width: 150, align: 'center' } ,
						        { label: '计划跟踪标识（01-未统计 02-已统计）', name: 'planmatchflag', index: 'planmatchflag', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#basicBussinessTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#basicBussinessTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#basicBussinessTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#basicBussinessTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/basicBussiness/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#basicBussinessTable").trigger("reloadGrid");
            });
        }
    }
}