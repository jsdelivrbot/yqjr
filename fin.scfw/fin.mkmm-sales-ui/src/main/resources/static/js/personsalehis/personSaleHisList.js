...
$(function () {
  $("#personSaleHisTable").jqGrid(
    {
      ...
      colModel: [
        { label: '序列', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '人员编号', name: 'personCode', index: 'personCode', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '预计数量', name: 'expectedNumber', index: 'expectedNumber', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#personSaleHisTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#personSaleHisTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#personSaleHisTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#personSaleHisTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/personSaleHis/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#personSaleHisTable").trigger("reloadGrid");
            });
        }
    }
}