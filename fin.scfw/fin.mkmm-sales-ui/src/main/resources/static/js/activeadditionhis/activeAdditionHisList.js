...
$(function () {
  $("#activeAdditionHisTable").jqGrid(
    {
      ...
      colModel: [
        { label: '序列', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '活动_ID', name: 'activeHisId', index: 'activeHisId', sortable: true, width: 150, align: 'center' } ,
						        { label: '活动编号', name: 'activeCode', index: 'activeCode', sortable: true, width: 150, align: 'center' } ,
						        { label: '附加贷编码', name: 'additionalCode', index: 'additionalCode', sortable: true, width: 150, align: 'center' } ,
						        { label: '附加贷名称', name: 'additionalName', index: 'additionalName', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeAdditionHisTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeAdditionHisTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeAdditionHisTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeAdditionHisTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeAdditionHis/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeAdditionHisTable").trigger("reloadGrid");
            });
        }
    }
}