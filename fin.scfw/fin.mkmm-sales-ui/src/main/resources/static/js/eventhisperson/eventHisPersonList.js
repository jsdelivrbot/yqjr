...
$(function () {
  $("#eventHisPersonTable").jqGrid(
    {
      url: apiEntityBasePath + "/pageByJqGrid",
      datatype: "json",
      mtype: 'GET',
      colModel: [
        { label: '事件序号', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '事件编号', name: 'eventCode', index: 'eventCode', sortable: true, width: 150, align: 'center' } ,
						        { label: '事件年度', name: 'eventYear', index: 'eventYear', sortable: true, width: 150, align: 'center' } ,
						        { label: '受理单位', name: 'eventApart', index: 'eventApart', sortable: true, width: 150, align: 'center' } ,
						        { label: '内容', name: 'eventText', index: 'eventText', sortable: true, width: 150, align: 'center' } ,
						        { label: '事件归属编号', name: 'code', index: 'code', sortable: true, width: 150, align: 'center' } ,
						        { label: '事件状态', name: 'eventStatus', index: 'eventStatus', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '操作状态', name: 'operateStatus', index: 'operateStatus', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#eventHisPersonTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#eventHisPersonTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#eventHisPersonTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#eventHisPersonTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/eventHisPerson/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#eventHisPersonTable").trigger("reloadGrid");
            });
        }
    }
}