  ...
  $("#activeProdTable").jqGrid(
    {
      ...
      colModel: [
        { label: '主键
', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeProdTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeProdTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeProdTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeProdTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeProd/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeProdTable").trigger("reloadGrid");
            });
        }
    }
}