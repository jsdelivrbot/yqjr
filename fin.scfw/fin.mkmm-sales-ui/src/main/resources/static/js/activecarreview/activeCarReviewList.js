...
$(function () {
  $("#activeCarReviewTable").jqGrid(
    {
      ...
      colModel: [
        { label: '序列', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '车型代码', name: 'moderCode', index: 'moderCode', sortable: true, width: 150, align: 'center' } ,
						        { label: '车型名称', name: 'moderName', index: 'moderName', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeCarReviewTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeCarReviewTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeCarReviewTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeCarReviewTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeCarReview/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeCarReviewTable").trigger("reloadGrid");
            });
        }
    }
}