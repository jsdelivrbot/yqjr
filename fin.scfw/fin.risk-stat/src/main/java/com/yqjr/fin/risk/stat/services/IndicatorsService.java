...
public class IndicatorsService extends BaseService<IndicatorsMapper, Indicators, Long> {
    ...
    public List<Indicators> selectByIndicatorname(Indicators it){
        return  mapper.selectByIndicatorname(it);
    }
    public boolean selMycode(String indicatorcode){
        if(mapper.selMycode(indicatorcode) == 0){
            return true;
        }else {
            return false;
        }
    }
    public boolean selByUpd(Indicators it){
        if(mapper.selByUpd(it) == 0){
            return true;
        }else {
            return false;
        }
