...
$(function () {
  $("#activeBusinessHisTable").jqGrid(
    {
      ...
      colModel: [
        { label: '主键', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '活动ID', name: 'activeHisId', index: 'activeHisId', sortable: true, width: 150, align: 'center' } ,
						        { label: '活动编号', name: 'activeCode', index: 'activeCode', sortable: true, width: 150, align: 'center' } ,
						        { label: '业务编码', name: 'businessCode', index: 'businessCode', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeBusinessHisTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeBusinessHisTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeBusinessHisTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeBusinessHisTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeBusinessHis/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeBusinessHisTable").trigger("reloadGrid");
            });
        }
    }
}