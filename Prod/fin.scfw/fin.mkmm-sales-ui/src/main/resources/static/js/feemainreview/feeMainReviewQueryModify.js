...
function func(businessCode) {
    ...
    $("#btnSubmit").on("click", function (e) {
        ...
        if (!flag) {
            ...
            var maxBillFee = formData["maxBillFee"];
            var billFee = formData["billFee"];
            if (parseInt(maxBillFee) < parseInt(billFee) || parseInt(billFee) < parseInt(0)) {
                layer.alert('调整金额应大于0，小于应付金额！');
            } else{
                var isNum = /(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/;
                if(!isNum.test(billFee)) {
                    layer.alert('金额格式不正确！');
                } else {
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        url: '/mkmm/sales/feeMainReviews/queryEdit?businessCode=' + businessCode,
                        ...
                        layer.alert(obj.code, function () {
                            history.go(-1);
                            commonForward('/mkmm/ui/feeMainReview/queryList');
                        });
                    }).always(function () {
                        // $("button").attr("disabled", false);
                        // commonForward("location.href=document.referrer")
                    });
                }
            }
        }
