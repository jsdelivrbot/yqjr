...
import com.yqjr.fin.scfw.meta.condition.DictCondition;
import com.yqjr.fin.scfw.meta.services.DictService;
...
public class DictRest extends BaseRest<DictService, Dict> {

    //region generated by CodeRobot

    private Logger logger = LoggerFactory.getLogger(getClass());

    @ApiOperation(value = "分页查询数据字典主表（参数：实体对象属性、pageNum、pageSize）", tags = "数据字典主表信息：DictRest")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "pageNum", value = "页数", required = true, dataType = "Integer"),
            @ApiImplicitParam(name = "pageSize", value = "每页记录数", required = true, dataType = "Integer")
    })
    @RequestMapping(value = "/page", method = RequestMethod.GET)
    public PageResultResponse<Dict> page(
            @RequestParam(value = "pageNum", required = true, defaultValue = "1") Integer pageNum,
            @RequestParam(value = "pageSize", required = false, defaultValue = "10") Integer pageSize,
            DictCondition condition) {

        List<Dict> list = baseService.selectList(condition, PageBounds.of(pageNum, pageSize));
        PageInfo<Dict> p = new PageInfo<>(list);
        return this.getPageResultResponse(Const.SUCCESS, Const.CODE_SUCCESS, null, p);

    }

    @ApiOperation(value = "分页查询数据字典主表信息（参数：实体对象属性、pageNum、pageSize）", tags = "数据字典主表信息：DictRest")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "pageNum", value = "页数", required = true, dataType = "Integer"),
            @ApiImplicitParam(name = "pageSize", value = "每页记录数", required = true, dataType = "Integer")
    })
    @RequestMapping(value = "/pageByJqGrid", method = RequestMethod.GET)
    public JQGridJsonResult<Dict> pageByJqGrid(
            @RequestParam(value = "pageNum", required = true, defaultValue = "1") Integer pageNum,
        ...
        PageInfo<Dict> pageList = new PageInfo<>(list);
        JQGridJsonResult<Dict> jqGridJson = new JQGridJsonResult<>();
