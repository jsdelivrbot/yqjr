...
import com.yqjr.fin.scfw.meta.condition.PostCondition;
import com.yqjr.fin.scfw.meta.services.PostService;
...
public class PostRest extends BaseRest<PostService, Post> {

    //region generated by CodeRobot

    private Logger logger = LoggerFactory.getLogger(getClass());

    @ApiOperation(value = "分页查询岗位表（参数：实体对象属性、pageNum、pageSize）", tags = "岗位表信息：PostRest")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "pageNum", value = "页数", required = true, dataType = "Integer"),
            @ApiImplicitParam(name = "pageSize", value = "每页记录数", required = true, dataType = "Integer")
    })
    @RequestMapping(value = "/page", method = RequestMethod.GET)
    public PageResultResponse<Post> page(
            @RequestParam(value = "pageNum", required = true, defaultValue = "1") Integer pageNum,
            @RequestParam(value = "pageSize", required = false, defaultValue = "10") Integer pageSize,
            PostCondition condition) {

        List<Post> list = baseService.selectList(condition, PageBounds.of(pageNum, pageSize));
        PageInfo<Post> p = new PageInfo<>(list);
        return this.getPageResultResponse(Const.SUCCESS, Const.CODE_SUCCESS, null, p);

    }

    @ApiOperation(value = "分页查询岗位表信息（参数：实体对象属性、pageNum、pageSize）", tags = "岗位表信息：PostRest")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "pageNum", value = "页数", required = true, dataType = "Integer"),
            @ApiImplicitParam(name = "pageSize", value = "每页记录数", required = true, dataType = "Integer")
    })
    @RequestMapping(value = "/pageByJqGrid", method = RequestMethod.GET)
    public JQGridJsonResult<Post> pageByJqGrid(
            @RequestParam(value = "pageNum", required = true, defaultValue = "1") Integer pageNum,
        ...
        PageInfo<Post> pageList = new PageInfo<>(list);
        JQGridJsonResult<Post> jqGridJson = new JQGridJsonResult<>();
