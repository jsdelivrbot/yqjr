...
$(function () {
  $("#activeAdditionTable").jqGrid(
    {
      ...
      colModel: [
        { label: '序列', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '活动_ID', name: 'activeId', index: 'activeId', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeAdditionTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeAdditionTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeAdditionTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeAdditionTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeAddition/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeAdditionTable").trigger("reloadGrid");
            });
        }
    }
}