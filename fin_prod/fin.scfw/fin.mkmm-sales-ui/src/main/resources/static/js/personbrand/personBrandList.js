...
$(function () {
  $("#personBrandTable").jqGrid(
    {
      ...
      colModel: [
        { label: '序列', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '城市名称', name: 'brandCode', index: 'brandCode', sortable: true, width: 150, align: 'center' } ,
						        { label: '城市编号', name: 'brandName', index: 'brandName', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#personBrandTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#personBrandTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#personBrandTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#personBrandTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/personBrand/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#personBrandTable").trigger("reloadGrid");
            });
        }
    }
}