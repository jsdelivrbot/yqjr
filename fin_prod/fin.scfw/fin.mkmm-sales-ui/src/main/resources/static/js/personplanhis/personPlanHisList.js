...
$(function () {
  $("#personPlanHisTable").jqGrid(
    {
      ...
      colModel: [
        { label: '序列', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '人员编号', name: 'personNo', index: 'personNo', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '数量', name: 'amount', index: 'amount', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#personPlanHisTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#personPlanHisTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#personPlanHisTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#personPlanHisTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/personPlanHis/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#personPlanHisTable").trigger("reloadGrid");
            });
        }
    }
}