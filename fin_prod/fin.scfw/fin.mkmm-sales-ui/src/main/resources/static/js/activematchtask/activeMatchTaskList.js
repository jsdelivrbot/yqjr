...
$(function () {
  $("#activeMatchTaskTable").jqGrid(
    {
      ...
      colModel: [
        { label: 'ID', name: 'id', index: 'id', sortable: true, width: 150, align: 'center' } ,
						        { label: '任务时间', name: 'taskDate', index: 'taskDate', sortable: true, width: 150, align: 'center' } ,
						        ...
						        { label: '是否执行', name: 'isExecute', index: 'isExecute', sortable: true, width: 150, align: 'center' } ,
  ...
  $("#activeMatchTaskTable").jqGrid('navGrid', $("#pager"), {
    edit: false,
    add: false,
    del: false,
    search: false,
  ...
  if (!shrinkToFit) {
    $("#activeMatchTaskTable").closest(".ui-jqgrid-bdiv").css({ 'overflow-x': 'scroll' });
  }
...
function update() {
  var ids = $("#activeMatchTaskTable").jqGrid("getGridParam", "selarrrow");// 获得选中行数据
  ...
  if (ids.length === 0) {
    ...
    var rowdata = $("#activeMatchTaskTable").jqGrid("getRowData", ids[0]);
    commonForward('/mkmm/ui/activeMatchTask/modify?id=' + rowdata.id)
  }
    ...
    if (ids.length === 0) {
        ...
        if (confirm("确定进行批量删除？")) {
            ...
            $.ajax({
                ...
                layer.alert(obj.msg);
                $("#activeMatchTaskTable").trigger("reloadGrid");
            });
        }
    }
}