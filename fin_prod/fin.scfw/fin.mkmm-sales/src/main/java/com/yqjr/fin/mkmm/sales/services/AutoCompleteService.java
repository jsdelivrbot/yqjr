...
public class AutoCompleteService {
    ...
    public List<AutoCompleteVo> autoComplete(String codeType,String name){
        ...
        if("Person".equals(codeType)){
            UserContext userContext1 = SessionHolder.getUserContext();
            PersonMainAndTeam personMainAndTeam=new PersonMainAndTeam();
            personMainAndTeam.setPersonName(name);
            personMainAndTeam.setCompany(company);
            personMainAndTeam.setAreaCode(userContext.getAreaCode());
            List<PersonMain> personMains=personMainService.queryPersonMainList(personMainAndTeam);
